...
def main():
    """
    Função principal do pipeline de extração de dados do Google Ads.
    
    Esta função orquestra todo o processo de extração, transformação e carregamento
    de dados do Google Ads:
    
    1. Configura a autenticação com a API do Google Ads
    2. Extrai dados de campanhas ativas e suas métricas
    3. Extrai dados de conversão associados às campanhas
    4. Processa e transforma os dados em um formato adequado para análise
    5. Combina dados de campanhas e conversões
    6. Normaliza e limpa os dados
    7. Carrega os dados processados em um banco de dados PostgreSQL
    ...
    spark = SparkSession.builder.appName("GoogleAdsInsights").getOrCreate()
